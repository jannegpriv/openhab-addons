...
public class LynkcoAPI {
    ...
    private static final String CLIENT_ID = "c3e13a0c-8ba7-4ea5-9a21-ecd75830b9e9";
    private static final String REDIRECT_URI = "msauth://prod.lynkco.app.crisp.prod/2jmj7l5rSw0yVb%2FvlWAYkK%2FYBwk%3D";
    private static final String REDIRECT_URI_ENCODED = "msauth%3A%2F%2Fprod.lynkco.app.crisp.prod%2F2jmj7l5rSw0yVb%252FvlWAYkK%252FYBwk%253D";
    ...
    private boolean postLogin(String username, String password, String xMsCpimTransValue, String xMsCpimCsrfToken)
        ...
        logger.debug("postLogin URL: {}", uri.toString());
        logger.debug("postLogin fields: signInName={}, password=(hidden)", username);
        logger.debug("postLogin cookies: {}", httpClient.getCookieStore().getCookies());
        logger.debug("postLogin form data: request_type={}, signInName={}", fields.get("request_type"),
                fields.get("signInName"));
        ...
        String formData = "request_type=RESPONSE&signInName=" + URLEncoder.encode(username, StandardCharsets.UTF_8)
        ...
        ContentResponse response = request.send();
        logger.debug("postLogin response status: {}", response.getStatus());
        logger.debug("postLogin response body: {}", response.getContentAsString());
    ...
    private @Nullable CombinedSigninResponse getCombinedSigninAndSignup(String csrfToken, String txValue,
        ...
        Request request = httpClient.newRequest(url).method(HttpMethod.GET).agent(USER_AGENT).followRedirects(false)
                ...
                .header("Referer", refererBaseUrl
                        ...
                        + codeChallenge + "&x-client-CPU=64&redirect_uri=" + REDIRECT_URI_ENCODED
                        + "&client-request-id=0207E18F-1598-4BD7-AC0F-705414D8B0F7&client_id=" + CLIENT_ID
                        + "&x-client-DM=iPhone&return-client-request-id=true&haschrome=1")
        ...
        logger.debug("Cookies being sent: {}", httpClient.getCookieStore().getCookies());
        logger.debug("REFERER HEADER CHECK: {}", request.getHeaders().get("Referer"));
        ContentResponse response = request.send();
        logger.debug("Response status: {}", response.getStatus());
        logger.debug("Response Location header: {}", response.getHeaders().get("Location"));
    ...
    public TokenResponse getTokensFromRedirectUri(String redirectUri, String codeVerifier) throws LynkcoApiException {
        try {
            ...
            if (query == null) {
                throw new LynkcoApiException("No query parameters in redirect URI",
                        LynkcoApiException.ErrorType.AUTHENTICATION_FAILED);
            }

            // Check for error first
            String error = getQueryParam(redirectUri, "error");
            if (error != null && !error.isEmpty()) {
                String errorDescription = getQueryParam(redirectUri, "error_description");
                throw new LynkcoApiException("Authentication failed: " + error + " - " + errorDescription,
                        LynkcoApiException.ErrorType.AUTHENTICATION_FAILED);
            }

            // Extract code parameter
            String code = getQueryParam(redirectUri, "code");
            if (code == null || code.isEmpty()) {
                throw new LynkcoApiException("No authorization code found in redirect URI",
                        LynkcoApiException.ErrorType.AUTHENTICATION_FAILED);
            }
            ...
            throw new LynkcoApiException("Failed to process redirect URI: " + e.getMessage(),
                    LynkcoApiException.ErrorType.AUTHENTICATION_FAILED);
        }
    }
}
